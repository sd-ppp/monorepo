...
const MIME_TO_EXTENSION = Object.entries(EXTENSION_TO_MIME).reduce((acc, [ext, mime]) => {
    if (!acc[mime]) {
        acc[mime] = ext;
    }
    return acc;
}, {});
export function encodeSvgData(svg) {
    if (typeof btoa === "function") {
        return btoa(svg);
    }
    return Buffer.from(svg, "utf8").toString("base64");
}
export function buildGenericFileThumbnail(extension) {
...
export function normaliseExtension(value) {
    if (!value)
        return undefined;
