...
import { buildGetImageParamsFromResources, buildGetMaskParamsFromResources } from "../../../../../../internals/ps-uxp/src/mesh/actions/photoshop-impls/getImage.helpers";
import { buildBoundaryUri, extractDocIdFromUris, parseContentResource, parseMaskResource } from "../../../resource-uris.js";
import { createResource, updateResource, resolveResourceBuffer as resolveSharedResourceBuffer } from "../../image-holder.js";
...
async function isPhotoshopSelectionEmpty() {
    try {
        ...
        if (typeof selection.isEmpty === "function") {
            return await selection.isEmpty();
        }
...
export async function resolveEffectiveBoundaryUri(params) {
...
function getEffectiveImageSize(mesh, requested) {
    ...
    const meshState = mesh?.store?.getState?.() ?? {};
    const workBoundaryMaxSizes = meshState?.workBoundaryMaxSizes ?? {};
...
export async function loadContentSnapshotJimp(mesh, boundaryUri, contentUri) {
...
export async function loadMaskSnapshotJimp(mesh, boundaryUri, maskUri) {
...
export function registerCreateFromCBMAction(context) {
    const { mcpMesh } = context;
    const mesh = mcpMesh;
    mcpMesh.implementAction("fileResource.createFromCBM", async (rawParams = {}) => {
        try {
            try {
            ...
            const thumbnail = typeof materialized.thumbnail === "string"
            ...
            try {
                console.log("[createFromCBM] response", JSON.stringify({
                    resource: resourceId,
                    width,
                    height,
...
//# sourceMappingURL=register-create-from-cbm.js.map