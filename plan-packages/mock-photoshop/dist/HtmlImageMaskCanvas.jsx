...
export const HtmlImageMaskCanvas = ({ shapes: providedShapes, stageRef, selectionRect: selectionRectProp, onSelectionChange, selectedLayerId: selectedLayerIdProp, onLayerSelect, onShuffleShapes, }) => {
    const isShapesControlled = typeof providedShapes !== 'undefined';
    const [internalShapes, setInternalShapes] = useState(() => providedShapes ?? generateShapes(SHAPE_COUNT, STAGE_WIDTH, STAGE_HEIGHT));
    useEffect(() => {
    ...
    const [internalSelectedLayer, setInternalSelectedLayer] = useState(orderedShapes[0]?.id ?? null);
    ...
    const commitSelection = useCallback((rect) => {
    ...
    const handleLayerSelect = useCallback((shapeId) => {
    ...
    return (<div style={{ display: 'flex', alignItems: 'flex-start', gap: 20 }}>
      <div style={{
            width: STAGE_WIDTH,
            height: STAGE_HEIGHT,
            borderRadius: 20,
        ...
        <Stage width={STAGE_WIDTH} height={STAGE_HEIGHT} ref={node => {
            if (stageRef) {
                stageRef.current = node;
            }
        }} onMouseDown={handlePointerDown} onTouchStart={handlePointerDown} onMouseMove={handlePointerMove} onTouchMove={handlePointerMove} onMouseUp={handlePointerUp} onTouchEnd={handlePointerUp} onMouseLeave={handlePointerUp}>
          <Layer>
            <Rect x={0} y={0} width={STAGE_WIDTH} height={STAGE_HEIGHT} fillLinearGradientStartPoint={{ x: 0, y: 0 }} fillLinearGradientEndPoint={{ x: STAGE_WIDTH, y: STAGE_HEIGHT }} fillLinearGradientColorStops={[0, BACKGROUND_COLORS[0], 1, BACKGROUND_COLORS[1]]}/>
            {shapes.map(shape => {
            const baseProps = {
                key: shape.id,
                x: shape.x,
            ...
            switch (shape.kind) {
                ...
                default:
                    return (<Rect {...baseProps} width={shape.size} height={shape.size} offset={{ x: shape.size / 2, y: shape.size / 2 }}/>);
            }
        })}
            {activeRect ? (<>
                <Rect name="selection-overlay" x={activeRect.x} y={activeRect.y} width={activeRect.width} height={activeRect.height} fill="rgba(62, 124, 240, 0.15)" stroke="rgba(62, 124, 240, 0.65)" strokeWidth={2} dash={[8, 6]} cornerRadius={6}/>
                <KonvaText name="selection-overlay" x={activeRect.x} y={Math.max(0, activeRect.y - 24)} text={`${Math.round(activeRect.width)} × ${Math.round(activeRect.height)}`} fontSize={16} fontFamily="Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" fill="rgba(27, 43, 82, 0.85)" padding={5}/>
              </>) : null}
          </Layer>
        </Stage>
      </div>
      <div style={{
        ...
        <div style={{
          ...
          图层
          <button type="button" onClick={handleShuffle} title="随机刷新画布" style={{
            border: 'none',
        ...
        <div style={{
          ...
          {orderedShapes.map((shape, index) => {
            ...
            return (<button key={shape.id} type="button" onClick={() => handleLayerSelect(shape.id)} style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 10,
                    width: '100%',
...
const LayerThumbnail = ({ shape, isSelected }) => {
    ...
    return (<div style={{
      ...
      <Stage width={THUMBNAIL_WIDTH} height={THUMBNAIL_HEIGHT}>
        <Layer>
          <Rect x={0} y={0} width={THUMBNAIL_WIDTH} height={THUMBNAIL_HEIGHT} fillLinearGradientStartPoint={{ x: 0, y: 0 }} fillLinearGradientEndPoint={{ x: THUMBNAIL_WIDTH, y: THUMBNAIL_HEIGHT }} fillLinearGradientColorStops={[0, BACKGROUND_COLORS[0], 1, BACKGROUND_COLORS[1]]}/>
          {(() => {
            switch (shape.kind) {
                case 'circle':
                ...
                case 'star':
                    return (<Star {...commonProps} numPoints={5} innerRadius={(shape.size / 4) * scale} outerRadius={(shape.size / 2) * scale}/>);
                default:
                    return (<Rect {...commonProps} width={shape.size * scale} height={shape.size * scale} offset={{ x: (shape.size * scale) / 2, y: (shape.size * scale) / 2 }}/>);
            }
        })()}
        </Layer>
      </Stage>
